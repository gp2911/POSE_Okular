...
public:
    ...
    //added by Ganesh P Kumar on 29/3/2014 2155 hrs IST
    KToggleAction * aRightToLeft;
    //modification ended
    ...
    , Okular::View( QString::fromLatin1( "PageView" ) )
{
    ...
    d->aTrimMargins = 0;
    d->aMouseNormal = 0;
    //added by Ganesh P Kumar on 29/3/2014 2156 hrs IST
    d->aRightToLeft = 0;
    //modification ends
    d->aMouseSelect = 0;
...
void PageView::setupViewerActions( KActionCollection * ac )
{
    ...
    /* Added by Ganesh P Kumar on 29/3/2014 2158 hrs IST */
    /* Incomplete: Need to figure out actions            */

    d->aRightToLeft  = new KToggleAction( i18n( "Right To Left" ), this );
    ac->addAction("view_right_to_left", d->aRightToLeft );
    connect( d->aRightToLeft, SIGNAL(toggled(bool)), SLOT(slotRightToLeftToggled(bool)) );
    d->aRightToLeft->setChecked( Okular::Settings::rightToLeft() );
    printf("Right to Left = %d\n", Okular::Settings::rightToLeft() ); 
    /* Modification ends                                 */
...
void PageView::updateActionState( bool haspages, bool documentChanged, bool hasformwidgets )
{
    ...
    if ( d->aRightToLeft )
        d->aRightToLeft->setEnabled( haspages );
...
// called by: notifySetup, viewportResizeEvent, slotViewMode, slotContinuousToggled, updateZoom
{
    ...
    const bool rightToLeft = Okular::Settings::rightToLeft();
    //modification ends    

    printf("rightToLeft = %d\n", rightToLeft);
    ...
    PageViewItem * currentItem = d->items[ qMax( 0, (int)d->document->currentPage() ) ];
        ...
        for ( iIt = d->items.constBegin(); iIt != iEnd; ++iIt )
        {
            ...
            if ( continuousView || rIdx == pageRowIdx )
            {
                ...
                {
		// Commented out by Ganesh P Kumar 
		...
		*/	
		//added by Ganesh P Kumar
                	int trueVal = (fullWidth / 2)-item->croppedWidth()-1;
			int falseVal = (fullWidth / 2) + 1;
			if(centerFirstPage && item->pageNumber() % 2 == 1){
				if(!rightToLeft)
					actualX = trueVal;
				else
					actualX = falseVal;
                	}
			else if(!centerFirstPage && item->pageNumber() % 2 == 0){
				if(!rightToLeft)
					actualX = trueVal;
				else
					actualX = falseVal;
...
//added by Ganesh P Kumar
void PageView::slotRightToLeftToggled( bool on )
{
    if ( Okular::Settings::rightToLeft() != on )
    {
        Okular::Settings::setRightToLeft( on );
        Okular::Settings::self()->writeConfig();
        if ( d->document->pages() > 0 )
        {	
            //----------------
